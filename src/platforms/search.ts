 ...
 * the web family's `CoreError` codes.
 * @module @agents-web-search/core/platforms/search
 */
 ...
 * (`WEB_PROVIDER_ERROR`).
 */
function classifyPlatformError(error: unknown, signal: AbortSignal, context: string): CoreError {
  const timeout = timeoutOf(signal, 'WEB_SEARCH_TIMEOUT')
  if (timeout !== undefined) return new CoreError('platform search timed out', 'WEB_SEARCH_TIMEOUT', { cause: timeout })
...
function parseByFormat(platform: Platform, body: string, url: string): PlatformSource[] {
  switch (platform.format) {
    case 'html':
      if (platform.selectors === undefined) throw new CoreError(`platform "${platform.id}" is missing html selectors`, 'WEB_PROVIDER_ERROR')
      return parseHtmlResults(body, platform.selectors, url)
    case 'json':
      if (platform.fields === undefined) throw new CoreError(`platform "${platform.id}" is missing json fields`, 'WEB_PROVIDER_ERROR')
      return parseJsonResults(body, platform.fields)
    case 'json-in-html':
      if (platform.jsonInHtml === undefined) throw new CoreError(`platform "${platform.id}" is missing jsonInHtml config`, 'WEB_PROVIDER_ERROR')
...
export async function searchPlatform(
  ...
  if (platform.format === 'rss') {
    ...
    if (platform.searchUrl === undefined) throw new CoreError(`platform "${platform.id}" has no searchUrl`, 'WEB_PROVIDER_ERROR')
