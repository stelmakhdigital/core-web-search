...
export class CachedHttpFetchProvider {
   ...
   */
  private async revalidate(url: URL, key: string, cached: StoredPage, signal?: AbortSignal): Promise<FetchResult> {
    using d = deadline(signal, this.limits.timeoutMs, 'WEB_FETCH_TIMEOUT')
  ...
  private async followAndRead(
    initialUrl: URL,
    signal: AbortSignal,
  ): Promise<{ result: FetchResult; etag?: string; lastModified?: string }> {
  ...
  private async readBody(response: Response, finalUrl: URL, signal: AbortSignal): Promise<FetchResult> {
    ...
    const body: FetchBody = kind === 'html' ? { kind: 'html', content } : { kind: 'text', content }
...
/**
 * Translate a thrown fetch/stream error into a `CoreError`, classified by the
 ...
 */
function translateAbortOrNetwork(error: unknown, signal: AbortSignal): CoreError {
  const timeout = timeoutOf(signal, 'WEB_FETCH_TIMEOUT')
  if (timeout !== undefined) return new CoreError('web fetch timed out', 'WEB_FETCH_TIMEOUT', { cause: timeout })
