/**
 * `MultiSearchProvider`: a `SearchProviderLike` that routes one search across
 ...
 * @module @agents-web-search/core/search/provider
 */

import { CoreError } from '../errors.ts'
import type { SearchRequest, SearchResult, SearchSource } from '../types.ts'
...
export class MultiSearchProvider {
  ...
  async search(request: SearchRequest, signal?: AbortSignal): Promise<SearchResult> {
    ...
    if (cached !== undefined && Date.now() - cached.createdAt < this.options.searchCacheTtlMs) {
      ...
      // rows always came from this provider's own `SearchSource[]`.
...
export function filterAndDedupe(sources: readonly SearchSource[]): SearchSource[] {
...
/** Translate an unknown thrown value into a `CoreError`. */
function toWebError(error: unknown, message: string): CoreError {
