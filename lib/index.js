...
// src/store/schema.ts
var WEB_STORE_SCHEMA_VERSION, WEB_STORE_SCHEMA, WEB_STORE_MIGRATIONS;
var init_schema = __esm({
  "src/store/schema.ts"() {
    "use strict";
    WEB_STORE_SCHEMA_VERSION = 2;
    WEB_STORE_SCHEMA = `
  ...
  "src/store/index.ts"() {
    ...
    WebStore = class {
      ...
      async recordPage(entry) {
        ...
        if (id > 0) return id;
        const row = db.prepare("SELECT id FROM web_pages WHERE normalized_url = ?").get(entry.normalizedUrl);
        return row?.id ?? 0;
      }
      /** Read one search record by id (5.5, get_search_content). */
      async getSearch(id) {
        const db = await this.ensureOpen();
        const row = db.prepare("SELECT * FROM web_searches WHERE id = ?").get(id);
        return row === void 0 ? void 0 : mapSearchRow(row);
      }
      /** Read one page record by id (5.5, get_search_content). */
      async getPage(id) {
        const db = await this.ensureOpen();
        const row = db.prepare("SELECT * FROM web_pages WHERE id = ?").get(id);
...
async function startCuratorServer(options) {
  ...
  async function handleRequest(req, res) {
    ...
    try {
      ...
      if (url.pathname === "/api/summarize" && req.method === "POST") {
        ...
        if (text === void 0) return sendJson(res, 404, { error: "entry not found", code: "WEB_NOT_AVAILABLE" });
        const key = `${kind}:${id}`;
        const cached = summaries.get(key);
        if (cached !== void 0) return sendJson(res, 200, { summary: cached });
        const prompt = `Summarize the following web result in 2-4 sentences (what it is, the key facts, and why an agent might have fetched it).
...
${text.slice(0, 2e4)}`;
  ...
  await new Promise((resolve2, reject) => {
    ...
    server.listen(options.port ?? 0, bind, () => resolve2());
  });
...
async function readEntries(store, maxEntries, summaries) {
...
function toSearchEntry(s, summary) {
  const sources = Array.isArray(s.sources) ? s.sources.length : 0;
  const firstSnippet = Array.isArray(s.sources) && s.sources.length > 0 ? s.sources[0]?.snippet ?? "" : "";
  const preview = (s.content !== void 0 && s.content.length > 0 ? s.content : firstSnippet).toString().slice(0, 300);
  return {
    kind: "search",
    id: s.id,
    title: s.query,
    when: s.createdAt,
    meta: `${s.engines.join(", ") || "unknown engine"} \xB7 ${sources} source(s)${s.truncated ? " \xB7 truncated" : ""}`,
    preview,
    ...summary !== void 0 ? { summary } : {}
  };
}
function toPageEntry(p, summary) {
  ...
  return {
    ...
    title: p.url,
    when: p.fetchedAt,
    meta: `HTTP ${p.statusCode} \xB7 ${p.bodyKind}${p.truncated ? " \xB7 truncated" : ""}`,
    preview,
    ...summary !== void 0 ? { summary } : {}
  };
}
async function entryText(store, kind, id) {
  if (kind === "search") {
    const searches = await store.recentSearches(500);
    const found2 = searches.find((s) => s.id === id);
    if (found2 === void 0) return void 0;
    const sources = (Array.isArray(found2.sources) ? found2.sources : []).map((s, i) => {
      const src = s;
      return `${i + 1}. ${src.title ?? ""}
   ${src.url ?? ""}
   ${src.snippet ?? ""}`;
    }).join("\n").slice(0, 2e4);
    return `Query: ${found2.query}
Engines: ${found2.engines.join(", ")}
${found2.content !== void 0 && found2.content.length > 0 ? `Merged answer:
${found2.content.slice(0, 4e3)}
...
${found.bodyKind === "text" ? found.body.slice(0, 2e4) : `[${found.bodyKind} content]`}`;
}
...
function isLoopback(host) {
  return host === void 0 || host === "127.0.0.1" || host === "::1" || host === "localhost";
}
function renderPage() {
  return `<!doctype html>
<html lang="en">
...
<script>
  ...
  function entryHtml(kind, e) {
    return '<div class="entry"><h3>' + escapeHtml(e.title) + '</h3>' +
      '<div class="meta">' + escapeHtml(e.meta) + ' \xB7 ' + new Date(e.when).toLocaleString() + '</div>' +
  ...
  async search(request, signal) {
    ...
    try {
      ...
      const result = {
        ...
        ...content !== void 0 ? { content } : {},
        truncated,
        ...searchId !== void 0 && searchId > 0 ? { searchId } : {}
      };
...
function cloneSearchResult(result) {
  return {
    ...
    ...result.enginesUsed !== void 0 ? { enginesUsed: [...result.enginesUsed] } : {},
    ...result.fromCache !== void 0 ? { fromCache: result.fromCache } : {},
    ...result.searchId !== void 0 ? { searchId: result.searchId } : {}
  };
}
  ...
  async fetchFresh(url, signal) {
    if (this.limits.github.enabled) {
      ...
      if (target !== void 0) {
        ...
        return pageId !== void 0 && pageId > 0 ? { ...result2, pageId } : result2;
        return result2;
      }
    }
    ...
    if (result.statusCode >= 200 && result.statusCode < 300) {
      ...
      return pageId !== void 0 && pageId > 0 ? { ...result, pageId } : result;
    }
    return result;
  }
  /**
...
function pageToResult(page) {
  return {
    ...
    ...page.id !== void 0 && page.id > 0 ? { pageId: page.id } : {}
  };
}
  ...
  return {
    ...
    execute: guarded(async (args, { signal }) => {
      ...
      for (const query of queries) {
        ...
        if (result.searchId !== void 0) searchIds.push(result.searchId);
...
[...truncated...]`;
      const header = `Fetched ${result.url} (HTTP ${result.statusCode}, ${result.body.kind}${result.fromCache ? ", cache" : ""}, ${content.length} chars${truncated ? ", truncated" : ""})` + (result.pageId !== void 0 ? ` [page id: ${result.pageId} \u2014 full document via get_search_content {source: 'page', id: ${result.pageId}}]` : "");
...
var GET_CONTENT_DEFAULT_LIMIT = 2e4;
var GET_CONTENT_MAX_LIMIT = 1e5;
var GET_CONTENT_MAX_MATCHES = 3;
var GET_CONTENT_MATCH_CONTEXT = 400;
function buildGetSearchContentTool(host) {
  return {
    name: "get_search_content",
    description: "Read the full cached content of a previous web_search answer or web_fetch document. Use the record id printed in the web_search / web_fetch output. Without findText returns a character window (offset/limit); with findText returns context windows around the first matches.",
    ...
    execute: guarded(async (args) => {
      const record = asRecord(args);
      const source = record["source"] === "search" || record["source"] === "page" ? record["source"] : void 0;
      if (source === void 0) {
        throw new CoreError("source must be 'search' or 'page'", "WEB_BAD_REQUEST");
      }
      const id = typeof record["id"] === "number" && Number.isInteger(record["id"]) && record["id"] >= 1 ? record["id"] : (() => {
        throw new CoreError("id must be a positive integer", "WEB_BAD_REQUEST");
      })();
      const findText = typeof record["findText"] === "string" && record["findText"].length > 0 ? record["findText"] : void 0;
      const offset = record["offset"] === void 0 ? 0 : typeof record["offset"] === "number" && Number.isInteger(record["offset"]) && record["offset"] >= 0 ? record["offset"] : (() => {
        throw new CoreError("offset must be a non-negative integer", "WEB_BAD_REQUEST");
      })();
      const limit = record["limit"] === void 0 ? GET_CONTENT_DEFAULT_LIMIT : typeof record["limit"] === "number" && Number.isInteger(record["limit"]) && record["limit"] >= 1 && record["limit"] <= GET_CONTENT_MAX_LIMIT ? record["limit"] : (() => {
      ...
      const header = `${source === "search" ? "Search" : "Page"} #${id} ${title} \u2014 ${total} chars stored${truncated ? " (truncated at fetch time)" : ""}`;
      if (total === 0) {
        return { text: `${header}
...
${windows.join("\n\n")}${more}`;
        const capped = text2.length > GET_CONTENT_MAX_LIMIT ? `${text2.slice(0, GET_CONTENT_MAX_LIMIT)}
[...truncated...]` : text2;
        return { text: capped };
      }
...
function buildCuratorTool(host) {
  const servers = /* @__PURE__ */ new WeakMap();
  return {
    name: "web_curator",
    description: "Manage the local curator UI: a token-protected 127.0.0.1 web page where a human can review recent web searches and fetched pages, generate LLM summaries, and discard entries. action 'start' returns the URL (with token) to open in a browser; 'status' reports the running server; 'stop' closes it.",
    parameters: {
      type: "object",
      properties: {
        action: {
    ...
    execute: guarded(async (args, _ctx) => {
      ...
      if (action === "status") {
        if (running === void 0) return { text: 'No curator server is running. Use action "start" first.' };
        return { text: `Curator running at ${running.url}` };
      }
      if (action === "stop") {
        if (running === void 0) return { text: "No curator server is running." };
        servers.delete(host);
        await running.close();
        return { text: `Curator server stopped (port ${running.port}).` };
      }
      if (running !== void 0) return { text: `Curator already running at ${running.url}` };
      const { startCuratorServer: startCuratorServer2 } = await Promise.resolve().then(() => (init_curator(), curator_exports));
      const handle = await startCuratorServer2({
        store: host.store,
        llm: host.llm,
      ...
      if (host.config.extended.curator.remote) {
        lines.push(
          "NOTE: remote curator access is configured but deferred in v0.1 \u2014 the server is bound to loopback only (127.0.0.1) and speaks plain HTTP with no TLS; keep it local and do not port-forward the token URL."
        );
      }
      return { text: lines.join("\n") };
    })
